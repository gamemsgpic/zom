...
void UiHud::Reset()
{
	...
	SetScore(sceneGame->GetScore());
	SetHiScore(sceneGame->GetMaxScore());
	SetAmmo(ammo, allammo);
	SetHp(Hp, maxHp);
	SetWave(0);
	SetZombieCount(0);
}
...
{
	//if (player->GetFire() == true || player->GetReload() == true)
	//{
		ammo = player->GetAmmo();
		allammo = player->GetAllAmmo();
		Hp = player->GetCurrentHp();
		SetAmmo(ammo, allammo);
		SetHp(Hp, maxHp);
		SetWave(sceneGame->GetWave());
		SetZombieCount(sceneGame->GetWZombieCount());
		SetScore(sceneGame->GetScore());
		SetHiScore(sceneGame->GetMaxScore());
	//}
}
